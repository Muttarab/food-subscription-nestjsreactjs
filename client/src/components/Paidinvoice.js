...
const Paidinvoice = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await axios.get(`http://localhost:8000/paidinvoice/getPaidinvoicebyClientId/${user.id}`)
            const responseimg = await axios.get(`http://localhost:8000/paidinvoice/getImg/${response.data.receipt}`)
    ...
    const createPaidinvoice = async (dispatch, paidinvoice) => {
        ...
        try {
            ...
            const result = await axios.post(`http://localhost:8000/paidinvoice/upload/${user.id}`,
                data, {
                headers: {
